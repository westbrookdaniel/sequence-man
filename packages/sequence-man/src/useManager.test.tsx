import { useManager, useFrameManager, usePause } from ".";
...
describe("useManager", () => {
  test("can run and swap scenes", async () => {
    const { result } = renderHook(() =>
      useManager({ scenes: [scene1, scene2] })
    );
  ...
  test("will maintain last frame if not looping", async () => {
    const { result } = renderHook(() => useManager({ scenes: [scene1] }));
  ...
  test("will loop with shouldLoop", async () => {
    const { result } = renderHook(() =>
      useManager({ scenes: [scene1], shouldLoop: true })
    );
  ...
  test("handle no scenes", async () => {
    const { result } = renderHook(() => useManager({ scenes: [] }));
...
describe("useFrameManager", () => {
  test("should run every frame", async () => {
    const { result } = renderHook(() => useFrameManager({ scenes: [scene1] }));
    const { rerender } = render(result.current.view);
    expect(screen.getByText("First")).toBeInTheDocument();
